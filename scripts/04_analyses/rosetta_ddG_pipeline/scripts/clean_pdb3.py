...
from amino_acids_update import longer_names
from amino_acids_update import modres
...
def download_pdb(pdb_id, dest_dir, remote_host=None):
    url = f'http://www.rcsb.org/pdb/files/{pdb_id.upper()}.pdb.gz'
    dest = f'{os.path.abspath(dest_dir)}/{pdb_id}.pdb.gz'
    ...
    result = subprocess.run(wget_cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)

    if os.path.exists(dest):
        return dest
    else:
        print("Error: didn't download file!")
        print(result.stderr.decode())  # Print error message if download fails
        return None
...
def open_pdb(name):
    ...
    if filename is not None:
        print(f"Found existing PDB file at {filename}")
    else:
        print(f"File for {name} doesn't exist, downloading from internet.")
        filename = download_pdb(name[:4].upper(), '.')
    ...
    if stem.endswith('.gz'):
        stem = stem[:-3]
    if stem.endswith('.pdb1'):
        stem = stem[:-5]
    if stem.endswith('.pdb'):
        stem = stem[:-4]

    if filename.endswith('.gz'):
        with gzip.open(filename, 'rt') as f:  # 'rt' mode reads as text
            lines = f.readlines()
    else:
...
print(filename_stem, "".join(chainid), f"{nres:5d}", flag_altpos, flag_insres, flag_modres, flag_misdns, flag_successful)
